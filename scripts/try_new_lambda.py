...
# Add new concurrent fetching functions
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=50)) as session:
        ...
        for mode in MODES:
            for api_region in REGIONS:
                ...
                # Fetch first 20 pages for each region/mode
...
# Modify the main handler to use async
async def update_leaderboards_async():
    """Main async function to update leaderboards"""
    try:
        players = await fetch_leaderboards()
        if players:
            table = get_dynamodb_resource().Table(get_table_name())
            current_time = Decimal(str(int(datetime.now(timezone.utc).timestamp())))
            
            # Process players in batches
            items_to_update = []
            for player in players:
                game_mode = "1" if player["mode"] == "battlegroundsduo" else "0"
                game_mode_server_player = f"{game_mode}#{player['server']}#{player['playername']}"
                
                items_to_update.append({
                    "GameModeServerPlayer": game_mode_server_player,
                    "GameModeServer": f"{game_mode}#{player['server']}",
                    "PlayerName": player['playername'],
                    "CurrentRating": Decimal(str(player['rating'])),
                    "CurrentRank": Decimal(str(player['rank'])),
                    "LastUpdated": current_time
                })
            
            # Update items in batches
            update_rating_histories(table, items_to_update, current_time)
...
def lambda_handler(event, context):
    """AWS Lambda handler"""
    try:
        asyncio.run(update_leaderboards_async())
        return {
            'statusCode': 200,
            'body': json.dumps('Successfully updated leaderboards')
        }
    except Exception as e:
        logger.error(f"Error in lambda_handler: {e}")
        return {
            'statusCode': 500,
            'body': json.dumps(f'Error updating leaderboards: {str(e)}')
        }
