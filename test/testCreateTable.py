...
class testLeaderboardGet(unittest.TestCase):
    ...
    def testPutItem(self):
        self.database.put_item(
            region="NA", player="nina", rating=1, rank=1, lastUpdate=1
        )
    ...
    def testRankScan2(self):
        ...
        self.database.put_item(
            region="NA", player="lii", rating=1, rank=2, lastUpdate=1
        )
        foo = self.database.table.scan(
            FilterExpression=Attr("Rank").eq(1),
        )
    ...
    def testRankScan9(self):
        ...
        for i in range(2, 8):
            self.database.put_item(
                region="NA", player=str(i), rating=1, rank=i, lastUpdate=1
            )
        foo = self.database.table.scan(
            FilterExpression=Attr("Rank").eq(1),
        )
