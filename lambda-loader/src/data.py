...
class RankingDatabaseClient:
    ...
    def get_time(self, region, region_name="Region", player_name="PlayerName"):
        response = self.table.get_item(
            Key={player_name: timeToken, region_name: region}
        )
    ...
    def put_items(
        ...
        for i, item in enumerate(items):
            ...
            if player in players:
                update_rating = self.__append_rating_to_list(
                    rating=data[player]["rating"], item=item
                )
    ...
    def __append_rating_to_list(self, rating, item):
        if type(rating) != int:
            raise Exception(
                "Rating:",
                rating,
                "was expected to be of type int, but was type",
                type(rating),
                ".",
            )

        if (
            "Ratings" not in item or type(item["Ratings"]) is not list
        ):  # Key check and type check.
            item["Ratings"] = []

        if (
            item["Ratings"] and item["Ratings"][-1] == rating
        ):  # List is not empty and No updates to ratings list, return.
            return False
